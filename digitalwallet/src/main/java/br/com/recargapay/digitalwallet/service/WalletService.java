...
public class WalletService {
	...
	public Transaction depositFundsOLD(Long walletId, BigDecimal amount, String requestId) {
	    // Verificar se já existe uma transação com o mesmo requestId
	    if (transactionRepository.existsByRequestId(requestId)) {
	        logger.info("Duplicate request detected with requestId: {}", requestId);
	        throw new IllegalArgumentException("Duplicate transaction: requestId already exists.");
	    }
	    ...
	    transaction.setRequestId(requestId); // Garantindo a idempotência
	...
	public Wallet depositFunds(Long walletId, BigDecimal amount, String requestId) {
	    ...
	    // Buscar a carteira
	    Wallet wallet = walletRepository.findById(walletId)
	            .orElseThrow(() -> new IllegalArgumentException("Carteira não encontrada com ID: " + walletId));
